...
import { addPasswordAsync,fetchPasswords} from '../redux/thunks/passwordThunk'
    ...
    return (
        <div className='flex flex-col w-100 p-5 border-1 border-gray-300 shadow-md bg-linear-to-b from-yellow-100 to-zinc-100 rounded-2xl '>
            ...
            <div className='flex'>
                ...
                <button onClick={() => { dispatch(toggleAdd(false)); dispatch(setForm({title:"", site: "", username: "", password: "", note: ""})); }} className='w-20 bg-amber-300 mr-2 px-2 py-1 text-zinc-700 rounded-full hover:bg-amber-400 hover:scale-105 cursor-pointer'>Cancel</button>
            </div>
        </div>
    )
}
